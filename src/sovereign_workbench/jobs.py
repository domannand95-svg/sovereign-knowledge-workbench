...
def enqueue(database: sqlite3.Connection, plugin_id: str, source: Path, source_sha256: str) -> str:
    identity = hashlib.sha256(f"{plugin_id}\0{source.resolve()}\0{source_sha256}".encode()).hexdigest()
    now = _now()
    database.execute(
        "INSERT OR IGNORE INTO jobs(job_id,plugin_id,source_path,source_sha256,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?)",
        (identity, plugin_id, str(source.resolve()), source_sha256, "pending", now, now),
    )
    database.commit()
    return identity
...
def run_pending(database: sqlite3.Connection, *, limit: int = 25) -> dict[str, int]:
    ...
    rows = database.execute(
        "SELECT job_id,plugin_id,source_path,source_sha256 FROM jobs WHERE status='pending' ORDER BY created_at,job_id LIMIT ?",
        (limit,),
    ).fetchall()
    counts = {"completed": 0, "failed": 0}
    for job_id, plugin_id, source_path, expected_hash in rows:
        database.execute(
        ...
        database.commit()
        try:
            result = run_plugin(plugin_id, Path(source_path))
